...
return [
    ...
    'require_user_agent_on_api_text' => 'Require a non-generic User-Agent on API requests',
    'require_user_agent_on_api_help' => 'When enabled, API requests must have a User-Agent that does not match a common scripted client (curl, Postman, python-requests, Go, OkHttp, Guzzle, etc.). External scripts and integrations using default User-Agents will be rejected with a 403. User-Agent headers can be spoofed, so treat this as a casual filter that can be helpful to track specific integration usage, rather than a hard security boundary.',
    'api_user_agent_required' => 'Non-generic User-Agent is required. The User-Agent on this request was rejected.',
