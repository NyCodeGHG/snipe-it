...
<div class="form-group {{ $errors->has((isset($fieldname) ? $fieldname : 'image')) ? 'has-error' : '' }}" id="image-upload">
    <label class="col-md-3 control-label" for="{{ (isset($fieldname) ? $fieldname : 'image') }}">{{ $label ?? trans('general.image_upload') }}</label>
    <div class="col-md-8">
        ...
        <label class="btn btn-sm btn-theme" aria-hidden="true">
            ...
            <input type="file" name="{{ (isset($fieldname) ? $fieldname : 'image') }}" class="js-uploadFile" id="uploadFile" data-maxsize="{{ Helper::file_upload_max_size() }}" accept="{{ $accept ?? 'image/gif,image/jpeg,image/webp,image/png,image/svg,image/svg+xml,image/avif' }}" style="display:none; max-width: 90%" aria-label="{{ (isset($fieldname) ? $fieldname : 'image') }}" aria-hidden="true">
        </label>

        <span class='label label-default' id="uploadFile-info"></span>

        <p class="help-block" id="uploadFile-status">{{ $help_text ?? trans('general.image_filetypes_help', ['size' => Helper::file_upload_max_size_readable()]) }}</p>
